...
class OrderViewSet(viewsets.ModelViewSet):
    ...
    def partial_update(self, request, pk=None):
        ...
        html_message = render_to_string('email.html', {
            ...
            'cake_names': cake_names
        })
        plain_message = strip_tags(html_message)
        from_email = 'avixpatil@gmail.com'
    ...
    # def schedule_mail(self, send_time, subject, plain_message, recipient_list):
    #     delay = (send_time - now()).total_seconds()
    #     scheduler.enter(delay, 1, send_mail , argument=(subject, plain_message, settings.EMAIL_HOST_USER, recipient_list))

    def schedule_mail(self, send_time, subject, plain_message, recipient_list, html_message=None):
        delay = (send_time - now()).total_seconds()
        scheduler.enter(delay, 1, self.send_email, argument=(subject, plain_message, recipient_list, html_message))

    def send_email(self, subject, plain_message, recipient_list, html_message=None):
        from_email = 'avixpatil@gmail.com'
    ...
    def delivery_tracking(self, request, id):
        ...
        try:
            ...
            # sweetspot_location = "CakeVilla, Kolhapur, Maharashtra, India"

            store = order.store
            if not store:
                return Response({"message": "Store not found for the order"}, status=status.HTTP_400_BAD_REQUEST)

            store_location = store.address
            delivery_location = order.delivery_address


            distance_matrix = gmaps.distance_matrix(store_location, delivery_location)
            ...
            if distance_matrix['status'] == 'OK':
                ...
                current_location = store_location  # Assuming the current location is the source for simplicity
                ...
                msg_conf = render_to_string('email_confirmation.html', {
                    'customer_first_name': order.customer.first_name,
                    'order_id': id,
                    'delivery_address': order.delivery_address,
                    'duration': duration
                })
                msg_conf_plain = strip_tags(msg_conf)
                self.schedule_mail(
                    now(),
                    f'Confirmation of Delivery: Order #{id}',
                    msg_conf_plain,
                    [order.customer.email],
                    msg_conf
                )

                msg_update = render_to_string('email_update.html', {
                    'customer_first_name': order.customer.first_name,
                    'order_id': id,
                    'delivery_address': order.delivery_address,
                    'duration': duration
                })
                msg_update_plain = strip_tags(msg_update)
                self.schedule_mail(
                    remainder_time,
                    f'Order Update: 5 Minutes Remaining for Order #{id}',
                    msg_update_plain,
                    [order.customer.email],
                    msg_update
                )

                msg_delivered = render_to_string('email_delivered.html', {
                    'customer_first_name': order.customer.first_name,
                    'order_id': id,
                    'delivery_address': order.delivery_address
                })
                msg_delivered_plain = strip_tags(msg_delivered)
                self.schedule_mail(
                    delivery_time,
                    f'Order Delivered: Order #{id}',
                    msg_delivered_plain,
                    [order.customer.email],
                    msg_delivered
                )
                ...
                scheduler.enterabs(delivery_time.timestamp(), 1, self.update_order_status,
                                   argument=(order, 'delivered'))
